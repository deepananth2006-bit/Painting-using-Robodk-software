...
4. Select ViewMake reference frames bigger (+) to increase the size of the reference frames
5. Select ViewMake reference frames smaller (-) to decrease the size of the reference frames
6. Select ViewShow/Hide text on screen (/) to show or hide the text on the screen
7. Optionally, rename any reference frame or object in the tree by selecting F2  
...
1. Hold ALT+Shift or select the highlighted button from the toolbar
2. Select the light blue plane (XZ plane of the TCP) and drag the TCP approximately towards the surface of the spray gun, as shown in the next imageGetting Started with RoboDK 7
...
7. Select Program Teach Target (Ctrl+T), or the corresponding button in the toolbar (as shown in the image). The target will be placed as a dependency of the active reference frame and will automatically remember the current robot position (cartesian and joints axes).   
...
11. 11. Select the Home target and the Approach target alternatively to see the robot moving between the two targets
...
4. Select Program Move Joint Instruction (or the corresponding button in the toolbar) Two instructions will be added automatically to tell the robot what tool frame and reference frames we are using  
**Note:** If no target is selected, a new target will be created at the same location of the robot.
5. Select the Approach target
6. Select Program Move Joint Instruction again - Double click the ApproachMove program and it will execute the program simulation. The simulation bar and an estimated cycle time will be displayed.
...
r.ProgSave(r"""C:/Users/Gnanendran/Documents/RoboDK""",r"""Mainprog""",True,r"""E:/RoboDK/Other/VSCodium/VSCodium.exe""")

```

**Developed by:** HarishPranav
...
## Results:
Thus, the program to paint the object for inspection using a Robot and RoboDK software has been successfully executed 
